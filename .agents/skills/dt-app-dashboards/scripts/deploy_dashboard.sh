...
usage() {
  cat <<EOF
Usage: $0 [--dry-run] <dashboard.json>
...
VALIDATION_OUTPUT=$(cat "$INPUT" | jq '{dashboard: .}' \
...
if [[ "$DRY_RUN" == "false" ]]; then
  rm -- "$INPUT"
  echo "Dashboard deployed successfully. The local file has been deleted. To make further changes, download the deployed dashboard first: dtctl get dashboard <id> -o json --plain > dashboard.json"
fi
